// src/pages/Admin/CreateGlobalExerciseSet.jsx
...
const STORAGE_KEY = "createGlobalExerciseSet";

export default function CreateGlobalExerciseSet() {
  const [title, setTitle] = usePersistedState(`${STORAGE_KEY}-title`, "");
  const [exercises, setExercises] = usePersistedState(`${STORAGE_KEY}-exs`, [
    {
      id: null,
      step: 1,
      title: "",
  ...
  const updateField = (idx, field, value) =>
    setExercises((exs) =>
      exs.map((ex, i) => (i === idx ? { ...ex, [field]: value } : ex))
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: bundle } = await axios.post(
        "/api/admin/bundles",
        { title },
      ...
      alert("Το global σετ δημιουργήθηκε!");
      setTitle("");
      setExercises([
        {
          id: null,
        ...
        <form onSubmit={handleSubmit} className="bundle-form">
          ...
          <div className="bundle-metadata">
            ...
            <p style={{ fontSize: 12, color: "#666", marginTop: 8 }}>
              Αυτό το σετ θα είναι ορατό σε όλους τους κλινικούς και μπορεί να
              ανατεθεί σε ασθενείς.
            </p>
          </div>
