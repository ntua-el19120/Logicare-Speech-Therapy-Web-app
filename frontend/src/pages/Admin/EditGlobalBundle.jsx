// src/pages/Admin/EditBundleAdmin.jsx
...
import "../../../style/EditBundleClinician.css"; // reuse same styles

export default function EditBundleAdmin() {
  const { id } = useParams();
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`/api/bundles/${id}`, { withCredentials: true })
      .then((res) => {
        const b = res.data;
        setTitle(b.title);
        setExercises(
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // 1) update bundle (title only; admins can only edit global)
      await axios.put(
      ...
      await Promise.all(
        exercises.map((ex) => {
          ...
          if (ex.mediaType === "audio") {
            if (ex.uploadFile instanceof File)
              form.append("audio", ex.uploadFile);
            if (ex.uploadPicture instanceof File)
      ...
      navigate("/admin/bundles", { replace: true });
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      ...
      <div className="bd-container ebc-page">
        <h1 className="bd-title">✏️ Επεξεργασία Global Bundle</h1>

        <form onSubmit={handleSubmit}>
          {/* Title row */}
          <div className="ebc-meta">
            <label htmlFor="bundle-title" className="ebc-label">
              Τίτλος
            </label>
            <input
              id="bundle-title"
              ...
              {ex.id != null && (
                <div className="bd-media">
                  <MediaViewer
                    videoUrl={
                      ex.mediaType === "video"
                        ? `/api/exercises/${ex.id}/video`
                        : null
                    }
                    audioUrl={
                      ex.mediaType === "audio"
                        ? `/api/exercises/${ex.id}/audio`
                        : null
                    }
                    pictureUrl={
                      ex.mediaType === "audio"
                        ? `/api/exercises/${ex.id}/picture`
                        : null
                    }
                  />
                </div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (f?.type.startsWith("video/"))
                        updateField(i, "uploadFile", f);
                      else
                        alert("Please choose a valid video file (.mp4, .webm, .ogg)");
                    }}
                    style={{ display: "none" }}
                  />
                </label>
                ...
                <div style={{ marginTop: "1rem", display: "flex", gap: "1rem" }}>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (f?.type.startsWith("audio/"))
                          ...
                          alert(
                            "Please choose a valid audio file (.mp3, .wav, .ogg, .m4a)"
                          );
                      }}
                      style={{ display: "none" }}
                    />
                  ...
                  <label className="bd-button">
                    ...
                    <input
                      ...
                      accept="image/*"
                      onChange={(e) =>
                        updateField(i, "uploadPicture", e.target.files[0])
                      }
                      style={{ display: "none" }}
                    />
                  </label>
          ...
          <div style={{ textAlign: "center" }}>
            <button type="submit" className="bd-button btn-edit">
              Save All Changes
            </button>
          </div>
        </form>
      </div>
      <FabBack to="/admin/bundles" />
    </>
  );
}
