// src/pages/Clinician/MyExercises.jsx
...
export default function MyExercises() {
  ...
  useEffect(() => {
    ...
    ; (async () => {
      try {
        ...
        const merged = uniqById([...(globals.data || []), ...(assigned.data || [])])
  ...
  const onDelete = async (bundle) => {
    if (bundle.global) return
    if (!window.confirm(`Delete “${bundle.title}”?`)) return
    try {
      setDeletingId(bundle.id)
      await axios.delete(`/api/bundles/${bundle.id}`, { withCredentials: true })
...
async function handleEdit(bundle) {
  try {
    // if it's global and we're a clinician → clone then edit the clone
    if (bundle.global && user?.type === 'clinician') {
      setCloningId(bundle.id)
      const { data } = await axios.post(
        `/api/bundles/${bundle.id}/clone-for-me`,
        {},
        { withCredentials: true }
      )
      navigate(`/edit-by-clinician/${data.id}`)
    } else {
      // own (non-global) bundle or admin
      navigate(`/edit-by-clinician/${bundle.id}`)
    }
  } catch (e) {
    alert(e.response?.data?.error || 'Could not open editor.')
  } finally {
    setCloningId(null)
  }
}
    ...
    <>
      ...
      <main className="bd-container me-content">
        ...
        <div className="me-titlebar">
          <h1 className="bd-title">Οι έτοιμες ασκήσεις μου</h1>
        </div>
      ...
      {/* Floating actions */}
      <Link to="/home-clinician" className="fab fab-secondary" aria-label="Αρχική">
        ← Αρχική
      </Link>
      <Link to="/create-clinician" className="fab fab-primary" aria-label="Νέο σετ">
        ＋ Νέο σετ
      </Link>
    </>
  )
